...
class OneStepCheckout {
	...
	/**
	 * Maybe redirect the user to the redirect url for the one step checkout order.
	 *
	 * @return void
	 */
	...
	public static function get_redirect_url_for_order( $order, $kec_unique_id ) {
		$max_attempts         = apply_filters( 'kec_one_step_redirect_wait_max_attempts', 20 ); // Try a maximum of 20 times to get the redirect url for the order.
		$sleep_time           = apply_filters( 'kec_one_step_redirect_wait_sleep_time_mu', 5 * 100000 ); // Sleep for 0.5 seconds between attempts.
		$attempt              = 0;
