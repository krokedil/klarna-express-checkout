...
class Session {
	...
	public function __construct() {
		add_action( 'woocommerce_checkout_update_order_review', array( $this, 'on_update_order_review' ) );
		add_action( 'woocommerce_thankyou', array( __CLASS__, 'unset_client_token' ), 1 );
		add_action( 'woocommerce_thankyou', array( __CLASS__, 'unset_klarna_address' ), 1 );
	}
	...
	public function on_update_order_review( $posted_data ) {
		...
		foreach ( $address_fields_map as $klarna_field => $wc_field ) {
			if ( ! $this->verify_address_field( $wc_field, $klarna_field, $parsed_data, $klarna_address, $ship_to_different_address ) ) {
	 ...
	 * @return bool
	 */
	private function verify_address_field( $wc_field, $klarna_field, $posted_data, $klarna_address, $ship_to_different_address ) {
		...
		if ( $shipping_value !== $klarna_value ) {
			...
			$this->set_field( 'shipping_' . $wc_field, $shipping_value );
			return false;
		}
		...
		if ( ! $ship_to_different_address && $billing_value !== $klarna_value ) {
			// Ensure we set the value to the customer.
			$this->set_field( 'billing_' . $wc_field, $billing_value );
			return false;
		}
	 ...
	 * @return void
	 */
	private function set_field( $field, $value ) {
